...
const RouteTable = (props) => {
    ...
    return ( 
        ...
        <SearchFilter filter = { globalFilter }
        setfilter = { setGlobalFilter } /> <CheckRole children = { 
            <Link to = "/dashboard/route/register" >
            <Button name = { 'Register new' } styles = { 'bg-primary text-white p-10 text-xs' }/> 
            </Link > }
        role = {['operator']} >
        </CheckRole>
         </section > <table {...getTableProps() }
        className = "w-full" >
        <thead > {
            headerGroups.map((headerGroup) => ( 
                ...
                className = "pb-10 " > {
                    headerGroup.headers.map((column) => ( 
                        ...
                        className = "text-left font-extrabold pr-5 border-b-2 pb-4" > { column.render('Header') } </th>))
                } </tr>
            )) } 
            </thead> 
            ...
            page.map((row) => {
                ...
                return ( 
                    <tr {...row.getRowProps() } > {
                        row.cells.map((cell) => {
                            return (
                                 <td {...cell.getCellProps() } className = "pr-5 py-4 border-b-2" > { cell.render('Cell') } </td>
                            )})
                    } </tr>);
            })
        } </tbody>
